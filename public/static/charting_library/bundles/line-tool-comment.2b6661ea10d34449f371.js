		...
		84738: (e, t, r) => {
				...
				_updateImpl(e) {
					...
					const o = this._labelRenderer.measure(),
						s = {
							...
							backgroundColor: (0, d.generateColor)(t.backgroundColor.value(), t.transparency.value()),
							innerWidth: o.width,
							innerHeight: o.height,
							borderRadius:
								Math.min(o.width, this._labelRenderer.lineHeight() + 2 * Math.round(t.fontsize.value() / 1.3)) / 2,
						};
					this._commentRenderer.setData(s),
						this._renderer.append(this._commentRenderer),
						1 === s.points.length &&
							this._renderer.append(
								this.createLineAnchor({ points: s.points.map(_.mapLineSourcePaneViewPointToLineAnchorPoint) }, 0)
							),
			...
			class w extends l.LineToolBalloon {
				...
				editableTextProperties() {
					return { text: this.properties().childs().text, textColor: this.properties().childs().color };
				}
				...
				_createPaneView() {
					this._setPaneViews([
						new R(this, this._model, this._openTextEditor.bind(this), this._closeTextEditor.bind(this)),
					]);
				}
